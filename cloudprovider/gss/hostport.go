...
func init() {
	hostPortPlugin := GssHostPortPlugin{
		mutex:        sync.RWMutex{},
		podAllocated: make(map[string]string),
		ipMap:        make(map[string]string),
	}
	gssProvider.registerPlugin(&hostPortPlugin)
}

func (hpp *GssHostPortPlugin) Name() string {
	return HostPortNetwork
}

func (hpp *GssHostPortPlugin) Alias() string {
	return ""
}

func (hpp *GssHostPortPlugin) OnPodAdded(c client.Client, pod *corev1.Pod, ctx context.Context) (*corev1.Pod, errors.PluginError) {
		...
		if ports, ok := containerPortsMap[container.Name]; ok {
			...
			for i, port := range ports {
				containerPort := corev1.ContainerPort{
					ContainerPort: port,
					HostPort:      port, //hostPorts[numToAlloc-1],
...
func (hpp *GssHostPortPlugin) OnPodUpdated(c client.Client, pod *corev1.Pod, ctx context.Context) (*corev1.Pod, errors.PluginError) {
	...
	nodeIp := getAddress(node, hpp.ipMap)
...
func (hpp *GssHostPortPlugin) OnPodDeleted(c client.Client, pod *corev1.Pod, ctx context.Context) errors.PluginError {
	log.Infof("Receiving pod %s/%s DELETE Operation", pod.GetNamespace(), pod.GetName())

	return nil
}

func (hpp *GssHostPortPlugin) Init(c client.Client, options cloudprovider.CloudProviderOptions, ctx context.Context) error {
	hpp.mutex.Lock()
...
func getAddress(node *corev1.Node, ipMap map[string]string) string {
	nodeIp := ""
